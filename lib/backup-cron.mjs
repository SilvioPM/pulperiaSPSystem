...
async function ejecutarPurga() {
  try {
    console.log('[Backup Cron] Ejecutando purga de datos antiguos...')
    await execAsync('node scripts/purge.js', { cwd: process.cwd() })
    console.log('[Backup Cron] Purga completada')
  } catch (error) {
    console.error('[Backup Cron] Error en purga:', error.message)
  }
}

// Respaldo semanal (domingos 02:00)
cron.schedule('0 2 * * 0', () => {
  generarRespaldoAutomatico()
  ejecutarPurga()
})

console.log(`[Backup Cron] Programado: respaldo + purga todos los domingos a las 02:00 → ${BACKUP_DIR}`)
